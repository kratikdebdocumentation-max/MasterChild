...
class AccountStateManager:
    ...
    def update_order_info(self, account_id: int, order_id: str, symbol: str, quantity: int, price: float) -> bool:
        ...
        try:
            ...
            account_name = self.cache[account_id]['account_name']
            applicationLogger.info(f"Updated {account_name} (ID: {account_id}) order info: {order_id} {symbol} {quantity}@{price}")
            
            return True
            
        except Exception as e:
            applicationLogger.error(f"Error updating account {account_id} order info: {e}")
            return False
